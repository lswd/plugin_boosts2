...
export default class PsFx extends cc.Component {
    ...
    @property(cc.Sprite)
    lianjie:cc.Sprite = null


    onLoad()
    {

        LocalizationManager.changeSprite(this.lianjie,"Combo2")
    ...
    play(audio:cc.AudioClip= null,spriteFrame = null)
    {
        ...
        if(spriteFrame){
            if(typeof(spriteFrame)=="string"){
                Common.setDisplay(this.sprite,spriteFrame)
            }else{
        ...
        // cc.log("[psfx] play : " ,  this.name,  dur);
        return new Promise((resolve,reject)=>{
            setTimeout(_=>{
