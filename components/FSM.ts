...
export default class FSM extends cc.Component
{
    ...
    {
        if(this._log)
            cc.log("[FSM]"+this.target.__classname__ + "("+ this.target.name+")"+" Add State :" ,id, name)
    ...
    enterState(stateId:number,params?)
    {
        ...
        if(this._log)
            cc.log("[FSM]"+this.target.__classname__ +" First State:" ,state.name)
    }
    ...
    {
        this.timeElapsed = 0
        cc.log(cc.js.formatStr("[FSM] %s reset currentState",this.target.__classname__))
        this.c.onExit();
        this.c.onEnter();
    }
    ...
    changeState(stateId:number,params?)
    {
        ...
        this.c = state;
        if(this._log)
            cc.log(cc.js.formatStr("[FSM] %s (%s): %s -> %s",this.target.__classname__,this.name,this.p.name , state.name))
